...
func Hash(plaintext string, keyLength uint32) (string, string, error) {
	generatedSalt, err := generateRandomBytes(keyLength)
	if err != nil {
		return "", "", err
	}
	generatedHash := generateArgon2idHash(plaintext, generatedSalt, keyLength)
...
func Verify(plaintext string, encodedHash string, encodedSalt string) error {
	...
	if err != nil {
		return fmt.Errorf("error while decoding hash: %s", err.Error())
	}
	salt, err := base64.RawStdEncoding.DecodeString(encodedSalt)
	if err != nil {
		return fmt.Errorf("error while decoding salt: %s", err.Error())
	}
	generatedHash := generateArgon2idHash(plaintext, salt, uint32(len(hash)))
	if bytes.Compare(generatedHash, hash) != 0 {
		return errors.New("provided plaintext string did not match provided hash and salt")
	}
	return nil
}
...
// using Argon2
func generateArgon2idHash(plaintext string, salt []byte, keyLength uint32) []byte {
	...
	keyLengthForAES := uint32(keyLength)
...
func generateRandomBytes(requiredLength uint32) ([]byte, error) {
	bytes := make([]byte, requiredLength)
	processedLength, err := rand.Read(bytes)
	if err != nil {
		return nil, err
	}
	if uint32(processedLength) != requiredLength {
		return nil, fmt.Errorf("required length of random bytes (%v) was different from the number of bytes processed (%v)", requiredLength, processedLength)
	}
	return bytes, nil
}
